...
登不上、要扫码：该源 `BLOCKED`，换下一公开源或停。禁止绕过登录、验证码、风控。

**Requests too frequent**、**Security Verification** 或验证码墙：不要把当前页再点开。立刻停点 → 冷却 60 秒 → 关掉当前页面/标签 → 新开搜索 URL。同一搜索词最多 2 次；仍墙则该词 `BLOCKED`，换下一词。协议全文在 `XHS-Topic-Pipeline.md`「频控」。禁止手动过验证码。频控 BLOCKED 的词不进入后续打开，也不要为它问 Jev。
